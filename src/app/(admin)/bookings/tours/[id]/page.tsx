'use client';
// src/app/(admin)/bookings/tours/[id]/page.tsx
...
import {
  BOOKING_STATUS_LABEL, BOOKING_STATUS_FLOW,
  QUOTE_STATUS_COLOR, PAYMENT_STATUS_COLOR,
  type TourBooking, type BookingQuote, type BookingPayment, type BookingStatus,
} from '@/types';
...
export default function TourDetailPage() {
  ...
  const [quoteCurr,     setQCurr]        = useState('LKR');
  const [quoteAmt,      setQAmt]         = useState('');
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: b }, { data: q }, { data: p }] = await Promise.all([
      sb.from('tour_bookings').select('*').eq('id', id).single(),
    ...
    if (!b) { router.push('/bookings/tours'); return; }
    const bk = b as TourBooking;
    setBooking(bk);
    ...
    if (!bk.viewed_at) {
      fetch('/api/bookings/update', {
        ...
        body: JSON.stringify({ booking_type: 'tour', booking_id: id, viewed_at: new Date().toISOString() }),
      }).catch(() => {});
    }
    setLoading(false);
  }, [id, router]);
  ...
  const saveChanges = async () => {
    ...
    try {
      ...
      else { setSaved(true); setTimeout(() => setSaved(false), 2500); load(); }
    } catch {
  ...
  const sendQuote = async () => {
    if (!quoteAmt || !booking) return;
    setSendingQuote(true); setQuoteError('');
    try {
      const res  = await fetch('/api/quotes/send', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          booking_type: 'tour', booking_id: id,
          booking_reference: booking.booking_reference,
          currency: quoteCurr, amount: parseFloat(quoteAmt),
          customer_email: booking.email, customer_name: booking.full_name,
        }),
      });
      const data = await res.json();
      if (!res.ok) { setQuoteError(data.error || 'Failed.'); return; }
      setQuoteModal(false); setQAmt(''); load();
  ...
  const sendPayLink = async () => {
    ...
    setSendingPay(true); setPayError('');
    try {
      ...
      setPayModal(false); setPayAmt(''); load();
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        ...
        subtitle={`Tour Booking · ${booking.full_name}`}
        actions={
          <Link href="/bookings/tours"
          ...
          <Card>
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-3">
              {([
                ...
                ['Listed Price', booking.tour_price_display|| '—'],
                ['Pickup',       booking.pickup_location],
                ['Start Date',   formatDate(booking.preferred_start_date)],
                ...
                <div key={l}>
                  ...
                  <p className="text-xs font-medium text-[#111]">{v}</p>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-5">
          <Card>
            ...
            <div className="mt-4">
              <Textarea label="Internal Notes" value={notes}
                ...
                rows={3} placeholder="Visible to admin team only…" />
            </div>
            {saveError && (
          ...
          <Card>
            ...
            <div className="space-y-3">
              {([
                ['Quote', booking.latest_quote_amount
                  ? formatCurrency(booking.latest_quote_amount, booking.latest_quote_currency || 'LKR') : '—'],
                ['Paid',  booking.total_paid_amount && booking.total_paid_amount > 0
      ...
      <Modal open={quoteModal} onClose={() => { setQuoteModal(false); setQuoteError(''); }} title="Send Quote">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <Select label="Currency" value={quoteCurr} onChange={e => setQCurr(e.target.value)}
              options={[{ value: 'LKR', label: 'LKR' }, { value: 'USD', label: 'USD' }]} />
            <Input label="Amount" type="number" value={quoteAmt}
              onChange={e => setQAmt(e.target.value)} placeholder="e.g. 85000" />
          </div>
          {quoteError && (
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button onClick={sendQuote} loading={sendingQuote} disabled={!quoteAmt}>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Input label="Amount" type="number" value={payAmt}
              onChange={e => setPayAmt(e.target.value)} placeholder="e.g. 85000" />
          </div>
          {payError && (
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button onClick={sendPayLink} loading={sendingPay} disabled={!payAmt}>
